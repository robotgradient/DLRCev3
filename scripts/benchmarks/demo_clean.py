...
def explore_workspace(robot, frame
                            , ltrack_pos=0, rtrack_pos=0, P=np.identity(3), marker_list = marker_total_list, delete_countdown = 0 , mapa = [], robot_trajectory = [],R=[],state_search = 2 , t1=0, 
                            t = None,feature_map = []):
    ...
    if time.time()-t  > 30:
        ...
        return "EXPLORE_WORKSPACE", frame, {"ltrack_pos": new_ltrack_pos, "rtrack_pos": new_rtrack_pos, "P": P , "marker_list": marker_total_list,
...
def select_and_go(robot,frame, cluster = 0,ltrack_pos=0, rtrack_pos=0,P = np.identity(3),R=[], mapa = [],tracker=None
                    ,img_res=np.asarray((640, 480)), atol=10,
                         vel_forward = 299, vel_rot = 100, atol_move_blind=90, 
...
def move_to_brick_blind_and_grip(robot, frame, R=[],ltrack_pos=0 ,
    rtrack_pos=0,marker_list=marker_total_list,mapa=[], vel=400, t=1700, cluster=None):
...
def A_star_move_to_box_blind(robot, frame, Map=[],cluster = 0, replan=1,
   ...
   ################ THIS IS ALLL
    ...
    vel_wheels, new_path = A_star_control(robot.position,obj,
